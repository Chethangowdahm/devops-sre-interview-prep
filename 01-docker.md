...
## Docker Build Flow — Step by Step
...
│  │  + context   │                                                            │
│  └──────┬───────┘                                                            │
│         │  Docker reads instructions top-to-bottom                           │
│         ▼                                                                    │
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │  INSTRUCTION 1: FROM golang:1.21-alpine                              │   │
│  │    └─► Pull base image from registry (if not cached)                 │   │
│  │    └─► Create Layer 0 (read-only)                                    │   │
│  │        Cache key: image digest                                        │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
│         │                                                                    │
│         ▼                                                                    │
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │  INSTRUCTION 2: COPY go.mod go.sum ./                                │   │
│  │    └─► Add files to container filesystem                             │   │
│  │    └─► Create Layer 1 (read-only)                                    │   │
│  │        Cache key: hash of go.mod + go.sum content                    │   │
│  │        ✓ CACHE HIT if go.mod unchanged → skip to next                │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
│         │                                                                    │
│         ▼                                                                    │
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │  INSTRUCTION 3: RUN go mod download                                  │   │
│  │    └─► Execute command in temporary container                        │   │
│  │    └─► Commit result as Layer 2 (read-only)                          │   │
│  │        Cache key: hash of Layer 1 + command string                   │   │
│  │        ✓ CACHE HIT if go.mod unchanged → skip (deps unchanged)       │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
│         │                                                                    │
│         ▼                                                                    │
│  ┌──────────────────────────────────────────────────────────────────────┐   │
...
│  │    └─► Create Layer 3 (read-only)                                    │   │
│  │        Cache key: hash of ALL source files                           │   │
│  │        ✗ CACHE MISS (source changed) → must rebuild from here        │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
│         │                                                                    │
│         ▼                                                                    │
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │  INSTRUCTION 5: RUN go build -o /server ./cmd/server                 │   │
│  │    └─► Compile application                                           │   │
│  │    └─► Create Layer 4 (read-only)                                    │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
│         │                                                                    │
│         ▼                                                                    │
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │  FINAL: Tag image → Store in local image store                       │   │
│  │    myapp:v1 = [ L0 ][ L1 ][ L2 ][ L3 ][ L4 ]                       │   │
│  │                                      THIN WRITABLE LAYER added at   │   │
│  │                                      runtime (container layer)       │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────────────┘

WHY LAYER ORDER MATTERS FOR CACHE:
  Slow path:  COPY . . → CACHE MISS → RUN go mod download (re-downloads!)
  Fast path:  COPY go.mod go.sum → RUN go mod download → COPY . . → build
  Savings: seconds vs minutes on every code change
```
...
```
  ...
  ┌─────────────────────────────────────┐
  │             STOPPED                  │
  │  (process exited, filesystem intact) │
  └──────────────────┬──────────────────┘
                     │ docker rm
                     ▼
              ┌──────────────┐
...
OOMKilled path:
  RUNNING → kernel detects memory limit exceeded → SIGKILL → STOPPED (exit 137)
  In K8s: kubelet detects → restarts pod → check: kubectl describe pod
```
...
WITHOUT multi-stage:
┌────────────────────────────────────────────────────────┐
│  golang:1.21 base          ~800 MB                      │
│  + go toolchain            (included)                   │
│  + your deps + source      +50 MB                       │
│  + compiled binary         +15 MB                       │
│  + test binaries           +20 MB                       │
│  ─────────────────────────────────────────             │
│  FINAL IMAGE:              ~880 MB  ← shipped to prod  │
└────────────────────────────────────────────────────────┘

WITH multi-stage:
┌────────────────────────────────────────────────────────┐
│  STAGE 1 (builder)         ~880 MB  ← NEVER SHIPPED    │
│  golang:1.21-alpine                                     │
│  + source + deps + build                               │
│    │                                                    │
│    │  COPY --from=builder /server /server              │
│    ▼                                                    │
│  STAGE 2 (runtime)         ~5 MB   ← SHIPPED TO PROD  │
│  gcr.io/distroless/static                               │
│  + compiled binary only                                 │
└────────────────────────────────────────────────────────┘

Result: 880 MB → 5 MB (99% reduction)
  Faster pulls, smaller attack surface, faster pod startup
```
